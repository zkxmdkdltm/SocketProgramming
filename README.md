...
-	Port 번호로 서버 생성 및 클라이언트 연결
 ...
 -> TCP통신의 Three Handshaking: 클라이언트로부터 연결이 올때까지 대기하고 연결이 올 시 accept() 메소드가 실행되어 연결이 완료된다.
...
-> 클라이언트가 보내는 메시지를 ‘in’ stream으로 수신 받고 해당 메시지에 대한 서버의 응답을 ‘out’ stream으로 전송한다.
...
<img width="446" alt="image" src="https://github.com/zkxmdkdltm/SocketProgramming/assets/102042061/810e1454-5f31-4c92-ab15-5f3a76eb48a9">
-> 클라이언트의 Request에 따른 Response 메시지를 stream에 write() 메소드를 이용하여 작성 후 flush() 메소드를 이용해 전송한다.
...
-> 통신이 끝난 후 close() 메소드를 통해 소켓, 서버, 그리고 input/output stream을 종료해준다
...
-> IP는 localhost의 IP를 사용하며 Port 번호는 서버의 Port 번호와 똑같이 설정 해준다.
...
-> 서버가 보내는 메시지를 ‘in’ stream으로 수신 받고 해당 메시지에 대한 클라이언트의 응답을 ‘out’ stream으로 전송한다.
...
-> write() 메소드로 out stream에 작성하고 flush()를 통해 Server로 메시지를 전송한다.
...
-> 통신이 끝난 후 close() 메소드를 통해 소켓과 input/output stream을 종료해준다
